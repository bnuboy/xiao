function Node(nodeId,x,y,nodeImg){
	...
	this.bitmap.regY = 25;
	var $this = this;
	this.bitmap.onPress = function(evt){handBitmapOnPress1(evt,$this);};
...
Node.prototype.addLine = function(line,p){
	this.lines.push({"line":line,"tou":p});
}
function handBitmapOnPress1(evt,$this){
	var target = evt.target;
	...
	evt.onMouseMove = function(e){
		...
		for(var i = 0; i < $this.lines.length;i ++){
			if($this.lines[i].tou == 1){
				$this.lines[i].line.setStartPoint(new Point(target.x,target.y));
				$this.lines[i].line.setEndPoint($this.lines[i].line.endPoint);
				$this.lines[i].line.drawLine();
			}else if($this.lines[i].tou == 2){
				$this.lines[i].line.setStartPoint($this.lines[i].line.startPoint);
				$this.lines[i].line.setEndPoint(new Point(target.x,target.y));
				$this.lines[i].line.drawLine();
			}
